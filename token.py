...
class Tree(object):
    ...
    def prefix_insert(self, phrase, oinfo, prefix_ignore=0):
        """ pass
        """
        exist = self.root.exist(phrase)
        if not exist: self.root.insert(phrase, oinfo)

        for idx in range(1, prefix_ignore + 1):
            if idx * 2 >= len(phrase): break
            key = '-' + str(idx) + phrase[idx:]
            exist = self.root.exist(key)
            if not exist: self.root.insert(key, oinfo)
